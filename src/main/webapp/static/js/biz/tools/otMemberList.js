...
App.controller("floatController", ['$scope','$http',  function ($scope,$http) {
    ...
    $scope.exportExportTask =function(){
        ...
        window.location.href = globalConfig.basePath + "/memberList/exportIvrTaskExcel?productChannel="+productChannel+"&status="+status;
    }
    //开机屏查询
   ...
   $scope.getTypeVersion = function(channel){
    ...
    $scope.downloadUserListFile = function (fileId, fileName) {
        ...
        if (fileName && fileName.length > 0) {
            window.open(globalConfig.basePath + "/memberList/downloadUserListFile?taskId=" + fileId + "&excelName=" +  encodeURI(encodeURI(fileName)));
        }
        /* alert("下载文件");
    ...
    $scope.preOperateUptade = function(productChannel,status,record){
        if(status==2 || status==3){
          ...
          //  $scope.canUpdate.endHour='18';
            ...
            $http.get(globalConfig.basePath + "/memberList/selectAllIvrWav"+"?productChannel="+productChannel
   ...
   /* $scope.validateRecord = function(){
        // 保存数据
        var url = null;
        url = globalConfig.basePath+"/memberList/invalid";
   ...
   //取消修改
    ...
    $scope.downloadTemplet  = function () {
        window.location.href = globalConfig.basePath + "/memberList/downloadTemplet";
    }
    ...
    $scope.pageQuerytask = function(pageNum){
        ...
        if(globalConfig.privilegeId!=''){
        	$scope.search.privilegeId = globalConfig.privilegeId;
        }
    ...
    $scope.saveTask = function () {
        ...
        if($scope.add.name==null || $scope.add.name==''){
            alert("名单名称不能为空");
            return;
        }
        if($scope.add.rule==2){
        	if($scope.add.qcPrivilegeId==null || $scope.add.qcPrivilegeId==''){
        		alert("请填写特权ID");
                return;
        	}
        ...
        if(xlsfile==null || xlsfile==''){
            alert("用户名单不能为空")
            return;
        }
        var updateMemberFileInput = $("#updateMemberFileInput").val();
        if(updateMemberFileInput==null||updateMemberFileInput==undefined||updateMemberFileInput==""){
        	alert("用户名单不能为空")
            return;
        }       
        ...
        //var wavName=$scope.taskList[$scope.video-1].wavName;
        //var wavName="wl"
        fileParams['name'] = encodeURI($scope.add.name);
        fileParams['rule'] = $scope.add.rule;
        var memberFile = $("#memberFile").val();
        fileParams['memberFile'] = memberFile;
        fileParams['privilegeId'] = globalConfig.privilegeId;
        fileParams['qcPrivilegeId'] = $scope.add.qcPrivilegeId;
        fileParams['fileName'] = encodeURI($scope.fileName);
        $('.ivr-button').hide();
        $('.ivr-processing-status span').text("0%");
        $('.ivr-processing-status').show();
        $scope.isCommitted = true;
        $.ajaxFileUpload({
            url: globalConfig.basePath + '/memberList/uploadMemberList?' + $.param(fileParams), //你处理上传文件的服务端
            secureuri: false,
    ...
    $scope.back =function(){
        window.location.href = globalConfig.basePath + "/tools/privilege";
    }
